...
le 15/05/2026 suit au meeting ; par l'ajout d'une composante continue

mais ça ne marche pas très bien, les reconstructions sont mauvaises !!!
"""
...
class AutoEncoder(nn.Module):
    def __init__(self):
        ...
        self.comp_continue = nn.Sequential(
            nn.Linear(LATENT_DIM, 128),
            nn.LeakyReLU(0.0),
            nn.Linear(128, 1)
        )
    ...
    def decode(self, z):
        ...
        out = self.dec_conv(h)
        # >>> ajout de la composante continue <<<
        out = out + self.comp_continue(z).unsqueeze(2)  # (batch, 1) -> (batch, 1, 1) pour broadcasting
        return out
